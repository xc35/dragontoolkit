...
/*     */ 
/*     */ public class ChiFeatureSelector extends AbstractFeatureSelector
...
/*  43 */     SortedArray list = computeTermCHI(getTermDistribution(indexReader, trainingSet), classPrior, docNum);
...
/*  66 */     SortedArray list = computeTermCHI(getTermDistribution(doctermMatrix, trainingSet), classPrior, docNum);
...
/* 100 */           chiVector.set(j, calChiSquare(termDistri.getInt(j, i), classVector.get(j), termVector.get(i), total));
/*     */         }
...
/*     */   {
/* 118 */     if ((t1sum == 0.0D) || (t2sum == 0.0D)) return 0.0D;
/*     */ 
/* 120 */     double expectedValue = t1sum * t2sum / total;
/* 121 */     double observedValue = t1t2occur;
/* 122 */     double chi = (observedValue - expectedValue) * (observedValue - expectedValue) / expectedValue;
/*     */ 
/* 124 */     expectedValue = t1sum * (total - t2sum) / total;
/* 125 */     observedValue = t1sum - t1t2occur;
/* 126 */     chi += (observedValue - expectedValue) * (observedValue - expectedValue) / expectedValue;
/*     */ 
/* 128 */     expectedValue = (total - t1sum) * t2sum / total;
/* 129 */     observedValue = t2sum - t1t2occur;
/* 130 */     chi += (observedValue - expectedValue) * (observedValue - expectedValue) / expectedValue;
/*     */ 
/* 132 */     expectedValue = (total - t1sum) * (total - t2sum) / total;
/* 133 */     observedValue = total - t1sum - t2sum + t1t2occur;
/* 134 */     chi += (observedValue - expectedValue) * (observedValue - expectedValue) / expectedValue;
/*     */ 
/* 136 */     return chi;
/*     */   }
/*     */ }
 ...
 * Qualified Name:     dragon.ir.classification.featureselection.ChiFeatureSelector
